...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 3. Typing Effect (Updated for Pivot)
    const typingText = document.getElementById('typing-text');
    const roles = [
        "Embedded Software Engineer",
        "Automation & Protocol Specialist",
        "RTOS & Linux Specialist",
        "Protocol Expert (Modbus / ARINC)"
    ];
    let roleIndex = 0;
    let charIndex = 0;
    ...
    if (menuTrigger && mobileMenu) {
        const toggleMenu = () => {
            ...
            // Icon transition
            const isMenuNowActive = isOpening;
            menuTrigger.innerHTML = isMenuNowActive ? 
        ...
        mobileMenu.querySelectorAll('a').forEach(link => {
            link.addEventListener('click', () => {
                ...
                document.body.style.overflow = '';
                menuTrigger.innerHTML = '<i data-lucide="menu" class="w-6 h-6"></i>';
                lucide.createIcons();
            });
        });
    }
